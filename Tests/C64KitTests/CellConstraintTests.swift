...
final class CellConstraintTests: XCTestCase {
    ...
    /// The seeded pseudo-random image, shared with the packing tests so both
    /// stages are measured against identical pixels. See `NoiseFixture`.
    private func noise(width: Int, height: Int) -> RGBBuffer {
        NoiseFixture.rgb(width: width, height: height)
    }
